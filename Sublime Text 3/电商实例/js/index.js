$(function(){
	...
	(function(){
		...
		function fnTab(oNav, aCon, sEvent){
			var aElem = oNav.children(); //找到onav第一层的子元素
			aCon.hide().eq(0).show();
	...
	(function(){
		...
		var oImg = oPrompt.find('.img_info');
	...
	//bbs移入展开,高亮显示
	(function(){
		var oBBsLi = $('.bbs ol li');
		oBBsLi.mouseover(function(){
			$(oBBsLi).removeClass('active').eq($(this).index()).addClass('active');
		})
	})();
	...
	(function(){
		...
		$('.hot_area li').mouseover(function(){
			if($(this).index() == 0){ return ;}//第一张图片没有这些效果
			$('.hot_area li p').remove(); //鼠标移入时，取消所有p标签元素。

			//console.log($(this).width()-12); //有12px的padding，因此宽度需要减去
			$(this).append('<p style="width:'+($(this).width()-12)+'px; height:'+($(this).height()-12)+'px;">'+arr[$(this).index()]+'</p>'); //给当前li元素添加半透明提示p，并设置p元素的宽高与当前li元素宽高一致.并对其中添加内容与arr中对应
		})
	})();
